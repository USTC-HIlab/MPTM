...
sub deal_pubmed {
	...
	@all_pmids = ();
	my $i = 0;
	open PUBMED, '<', "../files/pubmed/pubmed_$suffix.txt" or die $!;
	while (<PUBMED>) {
		if (/PMID- (.+)/) {
			my $pmid = $1;
			chomp($pmid);
...
print $file_suffix;
	...
	chdir '../lingpipe/demos/generic/bin/'
	  ...
	  my $cmd1 = qq(cmd_sentence_en_bio.bat "-inFile=../../../../files/init_file/$input" "-outFile=../../../../files/lingpipe_sen/$output_sentence");
	system($cmd1);
	my $cmd2 = qq("cmd_ne_en_bio_genetag.bat "-inFile=../../../../files/init_file/$input" "-outFile=../../../../files/lingpipe_ner/$output_entity");
	system($cmd2);
	chdir '../../../../bin/'
