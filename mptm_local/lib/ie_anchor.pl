		  ...
		  LAYER_2: for ( my $j = $i - 1 ; $j >= 0 ; $j-- ) {
					...
					|| $each_dep[$j] =~ /(.+?)\((.+?-\d+'?), $token\)/i )
				{
					...
					if (   $word !~ /(?:de|un|non)anchor/i
						&& $word =~ /anchor|inositol|GPI|glycosylphosphatidylinositol|mutation/i )
					{
						@substrate = push_finding( $pro1, @substrate )
						  ;    # push the found element into corresponding array
						@substrate =
					...
					# three layer
						...
						for ( my $k = $j - 1 ; $k >= 0 ; $k-- ) {
								...
								/(.+?)\((.+?-\d+'?), $word\)/i )
							{
								if (   $2 !~ /(?:de|un|non)anchor/i
									&& $2 =~ /anchor|GPI|glycosylphosphatidylinositol|mutation/i )
								{
									@substrate =
									  push_finding( $pro1, @substrate )
									  ; # push the found element into corresponding array
						...
						for ( my $k = $j + 1 ; $k < @each_dep ; $k++ ) {
							...
							{
								if (   $2 !~ /(?:de|un|non)anchor/i
									&& $2 =~ /anchor|GPI|glycosylphosphatidylinositol/i )
								{
									@substrate =
									  push_finding( $pro1, @substrate )
									  ; # push the found element into corresponding array
				...
				for ( my $j = $i + 1 ; $j < @each_dep ; $j++ ) {
						...
						|| $each_dep[$j] =~ /(.+?)\($token, (.+?-\d+'?)\)/i )
					{
						...
						if (   $relation !~ /conj_and/
							&& $word !~ /(?:de|un|non)anchor/i
							&& $word =~ /anchor|site|GPI|glycosylphosphatidylinositol/i )
						{
							@substrate = push_finding( $pro1, @substrate )
							  ; # push the found element into corresponding array
							@substrate =
...
elsif($each_dep[$i] =~ /nsubj\((?:proteinase|protein)-\d+'?, (PRO\d+)-(PRO\d+)-\d+'?\)/i ){
