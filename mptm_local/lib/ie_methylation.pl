...
sub site_pattern_methylation {
	...
	for ( my $i = 0 ; $i < @each_dep ; $i++ ) {
		...
		#(3)nsubj
		if ( $each_dep[$i] =~ /nsubj\(.*, (Lys|tsr)-\d+\)/i ) {
			my $site_name = $1;
			my $number='';
...
/\(((?:Ser|Thr|Tyr|\bT|\bY)\d+(\\\/(?:Ser|Thr|Tyr|\bT|\bY)\d+)?)-\d+'?, /i
		...
		if ( $each_dep[$i] =~
/, ((?:Ser|Thr|Tyr|\bT|\bY)\d+(\\\/(?:Ser|Thr|Tyr|tsr|\bT|\bY)\d+)?)-\d+'?\)/i
		  )
		{
			my $site_name = $1;
			@site = push_finding( $site_name, @site )
...
# (3) *(Ser|Thr|Tyr|serines?|threonines?|tyrosines?, \d+) or *(\d+, Ser|Thr|Tyr|serines?|threonines?|tyrosines?)
		...
		if ( $each_dep[$i] =~ /(SerP|ThrP|TyrP|Lys|lysines)/ ) {
			my $site_name = $1;
			@site = push_finding( $site_name, @site )
			  ;        # push the found element into corresponding array
		}
		...
		# this line
		if ( $each_dep[$i] =~
/.+?\((positions?|residues|cysteine|arginine?)-(\d+)'?, (\d+|one|two|three|four|five|six|seven|eight|nine|ten)-(\d+)'?\)/
		  )
		{
			my $res    = "residue";
			my $index1 = $2;
